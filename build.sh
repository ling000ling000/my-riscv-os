...
$CROSS_PREFIX-gcc -nostartfiles -nostdlib -ffreestanding -fno-builtin \
  -T./boot.lds \
  -Wl,-Map=$SHELL_FOLDER/output/lowlevelboot/lowlevel_fw.map -Wl,--gc-sections \
...
$CROSS_PREFIX-gcc -nostartfiles -nostdlib -ffreestanding -fno-builtin \
  -T./link.lds \
  -Wl,-Map=$SHELL_FOLDER/output/trusted_domain/trusted_fw.map -Wl,--gc-sections \
  $SHELL_FOLDER/output/trusted_domain/startup.o \
  -o $SHELL_FOLDER/output/trusted_domain/trusted_fw.elf
